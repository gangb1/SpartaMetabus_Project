...
public class TheStack : MonoBehaviour
{
    ...
    void ComboCheck()
    {
        comboCount++;     // 콤보 카운트 체크

        //콤보 카운트가 맥스 콤보를 넘으면 갱신
    ...
    void GameOverEffect()
    {
        ...
        //20개 이후에 있는 오브젝트들만 이펙트를 생성
        for (int i = 1; i < 20; i++)
        {
            if (childCount < i) break;

            GameObject go = transform.GetChild(childCount - i).gameObject;          //자식 오브젝트(블럭)저장

            if (go.name.Equals("Rubble")) continue;             //rubble이면 건들기X

            Rigidbody rigid = go.AddComponent<Rigidbody>();     //중력 컴포텐트 추가

            rigid.AddForce(
                (Vector3.up * Random.Range(0, 10f) + Vector3.right * (Random.Range(0, 10f) - 5f)) * 100f        //FORCE 주기
                );
